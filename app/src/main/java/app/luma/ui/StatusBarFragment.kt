...
class StatusBarFragment : Fragment() {
    ...
    fun Screen() {
        Column {
            ...
            val flashingState = remember { mutableStateOf(prefs.flashingSeconds) }
            ...
            ContentContainer {
                Column(verticalArrangement = Arrangement.spacedBy(33.5.dp)) {
                    ...
                    SelectorButton(
                        ...
                        onClick = {
                            findNavController().navigate(R.id.action_statusBarFragment_to_timeFormatFragment)
                        },
                    )
                    ToggleTextButton(
                        title = stringResource(R.string.status_bar_blinking_colon),
                        checked = flashingState.value,
                        onCheckedChange = {
                            flashingState.value = it
                            prefs.flashingSeconds = it
                        },
                        onClick = {
                            flashingState.value = !flashingState.value
                            prefs.flashingSeconds = flashingState.value
                        },
                    )
                }
            }
