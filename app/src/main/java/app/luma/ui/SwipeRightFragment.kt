            ...
            ContentContainer() {
                CustomScrollView {
                    for (action in Constants.Action.values()) {
                        val isSelected = prefs.swipeRightAction == action
                        val buttonText = when (action) {
                            Constants.Action.OpenApp -> "Open ${prefs.appSwipeRight.appLabel}"
                            else -> action.string()
                        }
                        SimpleTextButton(
                            title = buttonText,
        ...
        when(action) {
            Action.OpenApp -> {
                findNavController().navigate(
                    R.id.action_swipeRightFragment_to_appListFragment,
                    bundleOf("flag" to AppDrawerFlag.SetSwipeRight.toString())
                )
            }
            else -> {
